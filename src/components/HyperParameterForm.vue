<template>
  <div>
    ...
    <div v-else-if="item.type.indexOf('bool') > -1">
    ...
    <div v-else-if="item.type.indexOf('string') > -1">
      <input v-model="stringOptions" placeholder="Write different options separated by a comma (,)" style="width: 1000px" @input="updateString($event.target)" :name="item.name">
    </div>
  </div>
</template>
