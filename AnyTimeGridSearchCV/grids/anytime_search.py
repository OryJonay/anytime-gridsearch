...
def fit_and_save(estimator, X, y=None, groups=None, scoring=None, cv=None,
                 ...
                 return_train_score=True, error_score=0., parameters=dict(), uuid='', url='http://localhost:8000'):
    ...
    def fit(self, X=None, y=None, groups=None, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        if self.dataset is None and X is not None:
            ...
            try:
                numpy.savetxt(examples_file, X if type(X) not in [
                              coo_matrix, csr_matrix] else X.toarray(), delimiter=',')
            except TypeError:
                X.tofile(examples_file, sep=',')
            examples_file.seek(0)
            if y is not None:
                try:
                    numpy.savetxt(label_file, y if type(y) not in [
                                  coo_matrix, csr_matrix] else y.toarray(), delimiter=',')
                except TypeError:
